...
const Footer = () => {
  ...
  return (
    <footer data-nav-theme="light" className="bg-background h-screen flex flex-col justify-center">
      {/* Top area – logo + socials */}
      <div className="w-full px-[5vw] flex flex-col items-center flex-1 justify-end pb-[5vh]">
        <ScrollReveal>
          <p className="font-black text-[calc(3.5*var(--vf))] tracking-[0.2em] text-foreground mb-[5vh]">
            UNCHAIN
          </p>
        </ScrollReveal>
          ...
          {links.map((link) => (
            link.href.startsWith("/") ? (
              <Link
                key={link.label}
                to={link.href}
                className="text-[calc(0.9*var(--vf))] text-muted-foreground hover:text-foreground transition-colors"
              >
                {link.label}
              </Link>
            ) : (
      ...
      <div className="w-full px-[5vw] pb-[2vh] pt-[1vh] text-center">
        <p className="text-[calc(0.9*var(--vf))] text-muted-foreground">
